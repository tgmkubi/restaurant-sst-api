...
const getMenuHandler = async (event: IAPIGatewayProxyEventWithUserAndBody<null>): Promise<APIGatewayProxyResult> => {
    try {
        ...
        const menu = await tenantModels.Menu.findOne({
            _id: menuId,
            restaurantId,
            isActive: true
        }).lean().maxTimeMS(5000);
...
export const handler = lambdaHandlerTenantAuth(getMenuHandler, {
