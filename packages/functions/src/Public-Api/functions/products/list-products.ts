...
const listProductsHandler = async (event: IAPIGatewayProxyEventWithUser): Promise<APIGatewayProxyResult> => {
    try {
        ...
        const products = await tenantModels.Product.find({
            restaurantId: restaurantId,
            isActive: true
        })
        .select('_id name description price currency isActive createdAt restaurantId')
        .lean()
        .maxTimeMS(5000);
...
export const handler = lambdaHandlerTenantPublic(listProductsHandler, {
